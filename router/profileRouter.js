...
/*
    ...
    @access : PRIVATE
 */
router.put("/add_followers",authenticate,async (request, response) =>{
  try{
    
    let profileId = request.body.profileId;
    var userId = request.user.id;

    let profilet = await Profile.findOne({ _id: profileId });
    let profileuserId=profilet.user;
    var isfollow = profilet.followers.includes(userId);
   ...
   let profile_second=await Profile.findOne({user: userId});
    if(profile_second){
      profileId=profile_second._id;
      // console.log(profileId);


      var isfollowing = profile_second.following.includes(profileuserId);
