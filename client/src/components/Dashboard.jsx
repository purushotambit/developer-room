...
let Dashboard = () => {
   ...
   const getProfile = async() =>{
    let {status,data} = await axios.get("https://developer-room-yuba.onrender.com/api/profiles/me",{
      headers: {
 ...
 setLoading(false);
  ...
  let clickDeleteExperience = async(experienceId) => {
    const {data} = await axios.delete(`https://developer-room-yuba.onrender.com/api/profiles/experience/${experienceId}`,{
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("devroom")}`,
      },
  ...
  let clickDeleteEducation = async(educationId) => {
   const { data } = await axios.delete(
     `https://developer-room-yuba.onrender.com/api/profiles/education/${educationId}`,
     {
       headers: {
         "Content-Type": "application/json",
         Authorization: `Bearer ${localStorage.getItem("devroom")}`,
          ...
          {Object.keys(profile).length > 0 ? (
            <React.Fragment>
              <section>
                ...
                <button className="btn btn-dark btn-sm me-2" onClick={followerList} >
                          {profile.followers.length} Followers
              </button>

              <button className="btn btn-dark btn-sm me-2" onClick={followingList}>
